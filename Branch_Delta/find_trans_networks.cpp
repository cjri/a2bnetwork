...
void FindPairSets (run_params& p, const vector<int>& indivs, const vector< vector<ijlike> >& like_trans, const vector<varcount>& vc_multi, vector<tpairs>& trans_sets) {
    ...
    //New preliminary step: Calculate matrix of likelihoods that are OK.
    vector< vector<int> > OKlike;
    ConstructOKLike (p,like_trans,OKlike);
    /*if (p.diagnostic==1) {
        cout << "OKlike matrix:\n";
        PrintPossTransMatrix(OKlike);
    }*/
    ...
    for (int i=0;i<indivs.size();i++) {
        ...
        //Set up possible transmissions given the root.  Do we need allsets any more?
        vector< vector<tpair> > allsets;
        GetAllsets (root_index,indivs,OKlike,allsets);
        
        //Attempt to find a plausible index
        int possible=1;
        CheckPossible (possible,allsets);
        
        if (possible==1) {
            //Use set of possible transmissions to narrow down options
            ...
            while (fin==0) {
               /* for (int j=0;j<index.size();j++) {
                    ...
                    if (check_unlike==1) {
                        ...
                        if (check_span==1) {
                            ...
                            if (check_cycle==1) {
                                ...
                                if (check_seq==1) { //Keep this set of transmissions
                                    tpairs ts;
                                    ts.root=i;
                                    //cout << "K ";
                                    //cout << "Found\n";
                ...
                //Find next set to test
                int done=0;
                int up=index.size()-1;
                while (done==0) {
                    if (index[up]==allsets[up].size()-1) {
                        index[up]=0;
                        up--;
                        if (up<0) {
                            fin=1;
...
void GetAllsets (const int root_index, const vector<int>& indivs, const vector< vector<int> >& OKlike, vector< vector<tpair> >& allsets) {
    for (int j=0;j<indivs.size();j++) {
        if (j!=root_index) {
            ...
            allsets.push_back(as);
        }
    }
}

void CheckPossible (int& possible, const vector< vector<tpair> >& allsets) {
    for (int j=0;j<allsets.size();j++) {
        if (allsets[j].size()==0) {    //Nothing can transmit into an individual, therefore no complete network exists
            possible=0;
            break;
        }
    }
...
void ConstructOKLike (run_params& p, const vector< vector<ijlike> >& like_trans, vector< vector<int> >& OKlike) {
    for (int i=0;i<like_trans.size();i++) {
        int da=0;
        int db=0;
            vector<int> okl;
            for (int j=0;j<like_trans[i].size();j++) {
                da=like_trans[i][j].da+10;
                db=like_trans[i][j].db+10;
                if (p.tight==1) {
                    if (like_trans[i][j].lL_tot<p.threshold95[da][db]) {
                        okl.push_back(0);
                    } else {
                        okl.push_back(1);
                    }
                } else {
                    if (like_trans[i][j].lL_tot<p.threshold99[da][db]) {
                        okl.push_back(0);
                    } else {
                    okl.push_back(1);
                    }
            ...
            OKlike.push_back(okl);
        }
}

void CheckOKLikelihood(int& check_unlike, const vector<int>& index, const vector< vector<tpair> >& allsets, const vector< vector<int> >& OKlike) {
    for (int j=0;j<index.size();j++) {
        if (OKlike[allsets[j][index[j]].from][allsets[j][index[j]].to]==0) {
            check_unlike=0;
            break;
        }
    }
...
void CheckLikelihood(int& check_unlike, run_params& p, const vector<int>& index, const vector< vector<tpair> >& allsets, const  vector< vector<ijlike> >& like_trans) {
    //This subroutine now redundant.  Kept while verifying
    int da=0;
    int db=0;
    for (int j=0;j<index.size();j++) {
